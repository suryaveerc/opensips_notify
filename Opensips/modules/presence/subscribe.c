...
*/
		...
		if (updateSubscriptionInCache(subs, REMOTE_TYPE) < 0) {
...
//	LM_DBG("************HANDLE_SUBSCRIBE completed***********\n");
	...
	error: if (send_error_reply(msg, reply_code, reply_str) < 0) {
		LM_ERR("failed to send reply on error case\n");
	}
	error_free: if (subs.pres_uri.s)
		pkg_free(subs.pres_uri.s);
	if (subs.auth_rules_doc) {
...
/*
 ...
 void update_db_subs(shtable_t hash_table, int htable_size, int no_lock,
 ...
 version_col, socket_info_col, reason_col;
 ...
 //above three are used for update query too.
 ...
 char* presuri = malloc(s->pres_uri.len + 1);
 snprintf(presuri, s->pres_uri.len + 1, "%.*s", s->pres_uri.len,
 s->pres_uri.s);
 //"update active_watchers set expires=?,local_cseq=?,remote_cseq=?,version=?,status=?,reason=?,contact=? where presentity_uri = ? AND callid = ? AND to_tag= ? AND from_tag=?"
 if (updateResource(query_cols, query_vals, update_cols,
 update_vals, n_query_update, n_update_cols,
 SUBSCRIPTION, NULL, presuri) < 0) {
 ...
 if (insertResource(query_cols, query_vals, n_query_cols,
 SUBSCRIPTION, NULL) < 0) {
...
// "watcher_domain,event,event_id,local_cseq,remote_cseq,expires,status,reason,record_route,contact,local_contact,version,socket_info )
	if (insertResource(query_cols, query_vals, n_query_cols, SUBSCRIPTION, NULL)
			<= 0) {
		LM_ERR("unsuccessful sql insert\n");
		return -1;
	}
...
/*
 select presentity_uri,watcher_username,watcher_domain,to_user,to_domain,event,event_id,to_tag,from_tag,
 ...
 *//*
 ...
 LM_DBG("ENTER INTO RESTORE SUBS");
 ...
 event_col = n_result_cols++;
 event_id_col = n_result_cols++;
 totag_col = n_result_cols++;
 fromtag_col = n_result_cols++;
 ...
 if (getResource(NULL, NULL, 0, &result, SUBSCRIPTION, NULL,
 NULL) < 0) {
 ...
 LM_DBG("remote_cseq:%d", s.remote_cseq);
 ...
 LM_DBG("sockinfo_str:%s", sockinfo_str.s);
 if (sockinfo_str.s) {
 ...
 */
int refresh_watcher(str* pres_uri, str* watcher_uri, str* event, int status,
		str* reason) {
	...
	while (s) {
		if (s->event == ev && s->pres_uri.len == pres_uri->len
				...
				&& s->from_user.len == user.len
				&& strncmp(s->from_user.s, user.s, user.len) == 0
...
//	db_key_t result_cols[3];
	...
	/*	int len = subs->from_user.len + subs->from_domain.len + 2;
	 char user[len];
	 snprintf(user, len, "%.*s@", subs->from_user.len, subs->from_user.s);
	 snprintf(user + subs->from_user.len + 1, len, "%.*s", subs->from_domain.len,
	 subs->from_domain.s);*/
/*
	char temp_key[200] = "";
	char *tk = my_strcpy(temp_key, "", 0, 0);
...
/*
	...
	*(presuri + subs->pres_uri.len) = '\0';
*/
//	LM_DBG("presuri %s %d\n", presuri, strlen(presuri));
//	LM_DBG("BEFORE SELECT FROM WATCHERS.\n");
//select status,reason from watchers where presentity_uri=? AND watcher_username=? AND watcher_domain=? AND event=?
	if (getResource(db_keys, db_vals, n_query_cols, &result, WATCHER, NULL,
			NULL, NULL, 0) < 0) {
	...
	*found = 1;
	...
	if (row_vals[1].val.string_val) {
		...
		if (subs->reason.len == 0)
			...
			memcpy(subs->reason.s, row_vals[1].val.string_val,
					subs->reason.len);
		}
	} else {
		//LM_DBG("reason-------\n");
...
//insert into watchers (presentity_uri,watcher_username,watcher_domain,event,status,inserted_time,reason )
	if (insertResource(db_keys, db_vals, n_query_cols, WATCHER, NULL) <= 0) {
