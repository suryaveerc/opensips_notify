...
//	db_op_t  db_ops[2] ;
    ...
    int n = 0;
    //int user_col = 0, domain_col = 1, event_col = 2, etag_col = 3;
...
//    LM_DBG("cleaning expired presentity information\n");
    ...
    if (getResource(db_keys, db_vals, 1, &result, PRESENTITY, NULL, NULL, result_cols, n_result_cols) < 0) {
    ...
    for (i = 0; i < n; i++) {
        ...
        //LM_DBG("%s %s %s %s\n",user.s,domain.s,etag.s,event.s );
    ...
    if (deleteResource(db_keys, db_vals, 1, PRESENTITY, NULL, NULL) < 0)
...
//optional parameter - the URI of the sender, used by the BLA feature( event dialog;sla ).

int handle_publish(struct sip_msg *msg, char *sender_uri, char *str2) {
    //LM_DBG("************ Handle Publish Start****************** \n");
    struct sip_uri puri;
    str body;
    int lexpire;
