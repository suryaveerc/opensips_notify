...
//	LM_DBG("\n\n\n!!!!!!-%s-\n\n\n", pres_cache_key);
    ...
    if (fetch_values) {
        //LM_DBG("In checkPresentityInCache 2\n");
...
*/
        ...
        str body = str_init(reply->element[body_index]->str);
        presentity->body = body;

        if (reply->element[header_index]->len != 0) {
            str *extra_hdrs = NULL;
            extra_hdrs->s = (str *) pkg_malloc(sizeof(str));
            strcpy(extra_hdrs->s, reply->element[header_index]->str);
            extra_hdrs->len = reply->element[header_index]->len;
...
/*for (j=0;j< reply->elements; j++)
        ...
        j = 1;
        subs->local_contact.s = pkg_malloc(sizeof(char) * reply->element[j]->len);
        if (subs->local_contact.s) {
            strcpy(subs->local_contact.s, reply->element[j]->str);
            subs->local_contact.len = reply->element[j]->len;
        }else {
            LM_ERR("No more pkg memory\n");
            return -1;
        }
        j+=2;
        subs->record_route.s = reply->element[j]->len == 0 ? subs->record_route.len=0 : pkg_malloc(sizeof(char) * reply->element[j]->len);
        if (subs->record_route.s) {
            strcpy(subs->record_route.s, reply->element[j]->str);
            subs->record_route.len = reply->element[j]->len;
        }
        j+=2;
        subs->reason.s = reply->element[j]->len == 0 ? subs->reason.len =0 : pkg_malloc(sizeof(char) * reply->element[j]->len);
        if (subs->reason.s) {
            strcpy(subs->reason.s, reply->element[j]->str);
            subs->reason.len = reply->element[j]->len;
        }
        j+=2;
        subs->event_id.s = reply->element[j]->len == 0 ? subs->event_id.len = 0 : pkg_malloc(sizeof(char) * reply->element[j]->len);
        if (subs->event_id.s) {
            strcpy(subs->event_id.s, reply->element[j]->str);
            subs->event_id.len = reply->element[j]->len;
        }
        j+=2;
        subs->local_cseq = atoi(reply->element[j]->str);
        j+=2;
        subs->remote_cseq = atoi(reply->element[j]->str);
        j+=2;
        subs->status = atoi(reply->element[j]->str);
        j+=2;

        subs->version = atoi(reply->element[j]->str);
        j+=2;

        subs->expires = atoi(reply->element[j]->str);
        j+=2;

        subs->contact.s = pkg_malloc(sizeof(char) * reply->element[j]->len);
        strcpy(subs->contact.s, reply->element[j]->str);
        subs->contact.len = reply->element[j]->len;
        j+=2;

        sockinfo_str.s = pkg_malloc(sizeof(char) * reply->element[j]->len);
        strcpy(sockinfo_str.s, reply->element[j]->str);
        ...
        for (j=0;j< reply->elements; j+=2)
        {
            ...
            switch (simple_hash(reply->element[j]->str))
            {
                ...
                case _SOCKET_INFO:
                    ...
                    break;
                default:
                    LM_ERR("Unwanted VARIABLE found\n");
            }
        }
