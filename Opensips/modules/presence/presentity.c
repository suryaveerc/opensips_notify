	...
	//	LM_DBG("Publish for event dialog - try to send Notify for presence\n");
		...
		if (dialog_body) {
			*//* send Notify for presence *//*
			...
			if (publ_notify(presentity, pres_uri, 0, NULL, 0, dialog_body, 1)
					< 0) {
				LM_ERR("while sending Notify requests to watchers\n");
...
xmlNewProp(root_node, BAD_CAST "entity", BAD_CAST entity);
	...
	node = xmlNewChild(node, NULL, BAD_CAST "basic",BAD_CAST "open");
	if (node == NULL) {
		LM_ERR("while adding child\n");
		goto error;
	}
...
xmlNewProp(root_node, BAD_CAST "entity", BAD_CAST entity);
	...
	xmlDocDumpMemory(pres_doc, (xmlChar**) (void*) &body->s, &body->len);
