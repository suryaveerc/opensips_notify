...
str str__id_col = str_init("_id"); //Used for restheart api
...
inline void printf_subs(subs_t *subs)
{
    /*LM_DBG(
            ...
            subs->pres_uri.len, subs->pres_uri.s, subs->to_user.len,
            subs->to_user.s, subs->to_domain.len, subs->to_domain.s,
            subs->from_user.len, subs->from_user.s, subs->from_domain.len,
            subs->from_domain.s, subs->event->name.len, subs->event->name.s,
            get_status_str(subs->status), subs->expires, subs->callid.len,
            subs->callid.s, subs->local_cseq, subs->to_tag.len, subs->to_tag.s,
            subs->from_tag.len, subs->from_tag.s, subs->contact.len,
...
int build_str_hdr(subs_t *subs, int is_body, str *hdr, str *extra_hdrs)
{
    ...
    len = 7 /*Event: */+ subs->event->name.len + 4 /*;id=*/+ subs->event_id.len + CRLF_LEN + 10 /*Contact: <*/+
          subs->local_contact.len + 1/*>*/
          + ((subs->sockinfo && subs->sockinfo->proto != PROTO_UDP) ? 15/*";transport=xxxx"*/: 0) + CRLF_LEN +
          20 /*Subscription-State: */
          + status.len +
          ((subs->status == TERMINATED_STATUS) ? (10/*;reason=*/+ subs->reason.len) : 9/*expires=*/+ lexpire_len) +
          CRLF_LEN + (is_body ? (14 /*Content-Type: */+ subs->event->content_type.len + CRLF_LEN) : 0);
...
int get_wi_subs_db(subs_t *subs, watcher_t *watchers)
{
    ...
    //LM_DBG("Enter into get_wi_subs_db");
//	static db_ps_t my_ps = NULL;
    db_key_t query_cols[6], result_cols[6];
//	db_op_t  query_ops[6];
...
//	db_key_t result_cols[6];
    ...
    //char* presuri = malloc(subs->pres_uri.len + 1);
...
//	query_ops[n_query_cols] = OP_EQ;
    ...
    if (getResource(query_cols, query_vals, n_query_cols, &result, SUBSCRIPTION, NULL, presuri, result_cols,
                    n_result_cols) < 0) {
...
//		free(presuri);
    ...
    LM_WARN(" getResource took %ld millisecond\n", msec_used / 1000);
    *///free(presuri);
    if (result == NULL) {
        return 0;
    }
    ...
    if (result->n <= 0) {
        //LM_DBG("The query in db table for active subscription" " returned no result\n");
        free_result(result);
        return 0;
    }
...
/*
	...
	while (s->next) {
		...
		if (s->event == subs->event->wipeer
				...
				&& strncmp(s->pres_uri.s, subs->pres_uri.s, subs->pres_uri.len)
						== 0) {
			if (add_watcher_list(s, watchers) < 0) {
				LM_ERR("failed to add watcher to list\n");
...
*/
    ...
    notify_body = create_winfo_xml(watchers, version_str, subs->pres_uri, subs->event->wipeer->name, state);
...
int add_waiting_watchers(watcher_t *watchers, str pres_uri, str event)
{
    //LM_DBG("Enter into add_waiting_watchers.");
//	static db_ps_t my_ps = NULL;
    watcher_t *w;
    ...
    //char* presuri = malloc(pres_uri.len + 1);
    ...
    *(presuri + pres_uri.len) = '\0';
    ...
    /*wuser_col=n_result_cols++;
     wdomain_col=n_result_cols++;*/
    ...
    long secs_used, msec_used;
    gettimeofday(&start, NULL);
    */if (getResource(query_cols, query_vals, n_query_cols, &result, WATCHER, NULL, presuri, result_cols,
                      n_result_cols) < 0) {
...
*/
    ...
    for (i = 0; i < result->n; i++) {
        ...
        to64frombits((unsigned char *) w->id.s, (const unsigned char *) w->uri.s, w->uri.len);
...
static inline db_res_t *
pres_search_db(struct sip_uri *uri, str *ev_name, int *body_col, int *extra_hdrs_col, int *expires_col, int *etag_col)
{
    /*	static db_ps_t my_ps = NULL; */
    ...
    int n_query_cols = 0;
//	int i;
/*	 LM_DBG("********* USER and DoMAIN:%s@%s\n",uri->user.s,uri->host.s);
	 strcpy(pres_uri,uri->user.s);
	 strcpy(pres_uri,"@");
	 strcpy(pres_uri,uri->host.s);

	 LM_DBG("Created URI: %s\n",pres_uri);
*/
...
//select body,extra_hdrs,expires,etag from presentity where domain=? AND username=? AND event=? order by received_time

    /*struct timeval start, end;
    long secs_used, msec_used;
    gettimeofday(&start, NULL);
    */if (getResource(query_cols, query_vals, n_query_cols, &result, PRESENTITY, uri->user.s, NULL, result_cols,
                      n_result_cols) < 0) {
    ...
    LM_WARN(" getResource took %ld millisecond\n", msec_used / 1000);
    */if (!result) {
        //LM_DBG("No result returned.\n");
        return NULL;
    }
    //int count = result->n;
    /*LM_DBG("#########COUNT OF ROWS: %d#########", count);
    int i,j = 0;
    for (i = 0; i < count; i++) {
        j = 0;
        LM_DBG("!!!!!!!!!!!!!!%s", result->rows[i].values[j++].val.string_val);
        LM_DBG("!!!!!!!!!!!!!!%d", result->rows[i].values[j++].val.int_val);
        LM_DBG("!!!!!!!!!!!!!!%s", result->rows[i].values[j++].val.string_val);
        if (result->rows[i].values[j].val.string_val != NULL)
            LM_DBG("!!!!!!!!!!!!!!%s",
...
str* get_presence_from_dialog(str* pres_uri, struct sip_uri* uri,
	...
	if (p == NULL) {
		LM_DBG(
				"No record exists in hashtable, pres_uri=[%.*s] event=[dialog]\n",
				pres_uri->len, pres_uri->s);
		if (!fallback2db)
			return NULL;
	}

	result = pres_search_db(uri, &((*dialog_event_p)->name), &body_col,
			&extra_hdrs_col, &expires_col, &etag_col);
	if (result == NULL)
		return NULL;
...
str *
get_p_notify_body(str pres_uri, pres_ev_t *event, str *etag, str *publ_body, str *contact, str *dbody, str *extra_hdrs,
                  free_body_t **free_fct, int from_publish)
{
...
//	pres_entry_t* p;
    ...
    //pres_cache_key is set from the PUBLISH flow. For SUBSCRIBE flow, set here.

    char temp_key[200] = "";
    char *tk = my_strcpy(temp_key, "", 0, 0);
    char *p_key = tk;
    tk = my_strcpy(tk, PRESENTITY_SET_PREFIX, 0, 0);
    tk = my_strcpy(tk, "-", 0, 0);
    tk = my_strcpy(tk, pres_uri.s, 1, pres_uri.len);
    tk = my_strcpy(tk, ":", 0, 0);
    tk = my_strcpy(tk, event->name.s, 1, event->name.len);
    *tk = '\0';

    //LM_DBG("!!!!!!!!!!!!!!!!!!!!!!!-%s-", p_key);

    /* search in hash table if any record exists */
    //LM_DBG("LET'S SEARCH THE CACHE.\n");

    found_in_cache = fetchPresentityFromCache(p_key, &body_col, &extra_hdrs_col, &expires_col, &etag_col, &result);

    if (found_in_cache <= 0) {
        //LM_DBG("Nothing found in Cache. Search DB.\n");
    }
...
//	if( !etag && p== NULL){
    ...
    //LM_DBG("RETURN AFTER FUNCTION CALL 1");
    if (result == NULL)
        return NULL;
    //LM_DBG("RETURN AFTER FUNCTION CALL 2");
    if (result->n <= 0) {
        //LM_DBG("RETURN AFTER FUNCTION CALL 3");
        /*	LM_DBG(
                    "The query returned no result: [username]='%.*s'" " [domain]='%.*s' [event]='%.*s'\n",
...
//		result= NULL;
        ...
        LM_DBG(" ---------------------------------PRESENTITY FOUND---------------------------------\n");
        /*if (!found_in_cache) {
            // if presentity not found in cache, update the cache with the data fetched from DB.
            upsertPresentityInCache(&p_key, &pres_uri, &event->name, &result);
        }*/
        n = result->n;
        ...
        if (etag != NULL) {
            ...
            for (i = 0; i < n; i++) {
                ...
                if ((etags.len == etag->len) && (strncmp(etags.s, etag->s, etags.len) == 0)) {
                    //LM_DBG("found etag\n");
...
//		result= NULL;
    ...
    if (local_dialog_body && local_dialog_body != FAKED_BODY && local_dialog_body->s) {
...
dlg_t *build_dlg_t(subs_t *subs)
{
    ...
    if (subs->record_route.s && subs->record_route.len) {
        if (parse_rr_body(subs->record_route.s, subs->record_route.len, &td->route_set) < 0) {
...
//	db_op_t  query_ops[7];
    ...
    //LM_DBG("\nquerying database table = active_watchers\n");

//	char* presuri = malloc(pres_uri->len + 1);
    char presuri[pres_uri->len + 1];
    memcpy(presuri, pres_uri->s, pres_uri->len);
    *(presuri + pres_uri->len) = '\0';
    ...
    //Commmented for MOngoDB. Unable to send not equals.
    /*else {
...
//Maintain this order for MYSQL-REST
    /*from_user_col = n_result_cols++;
    from_domain_col = n_result_cols++;
    to_user_col = n_result_cols++;
    to_domain_col = n_result_cols++;
    event_id_col = n_result_cols++;
    to_tag_col = n_result_cols++;
    from_tag_col = n_result_cols++;
    callid_col = n_result_cols++;
    cseq_col = n_result_cols++;
    contact_col = n_result_cols++;
    record_route_col = n_result_cols++;
    expires_col = n_result_cols++;
    reason_col = n_result_cols++;
    version_col = n_result_cols++;
    sockinfo_col = n_result_cols++;
    local_contact_col = n_result_cols++;*/

    //Maintain this order for MONGODB_RESTHEART
...
*/
    ...
    if (result->n <= 0) {
        /*LM_DBG(
                "The query for subscribtion for [uri]= %.*s for [event]= %.*s" " returned no result\n",
    ...
    for (i = 0; i < count; i++) {
        ...
        if (sockinfo_str.s) {
            ...
            s.sockinfo = grep_sock_info(&host, (unsigned short) port, (unsigned short) proto);
        }
        //LM_DBG("sockinfo: %s", sockinfo_str.s);
...
int update_in_list(subs_t *s, subs_t *s_array, int new_rec_no, int n)
{
    ...
    for (i = 0; i < n; i++) {
        ...
        if (ls->callid.len == s->callid.len && strncmp(ls->callid.s, s->callid.s, s->callid.len) == 0 &&
            ls->to_tag.len == s->to_tag.len && strncmp(ls->to_tag.s, s->to_tag.s, s->to_tag.len) == 0 &&
            ls->from_tag.len == s->from_tag.len && strncmp(ls->from_tag.s, s->from_tag.s, s->from_tag.len) == 0) {
        ...
        while (s->next) {
            ...
            if ((!(s->status == ACTIVE_STATUS && s->reason.len == 0
                    ...
                    || (sender && sender->len == s->contact.len
                            && strncmp(sender->s, s->contact.s, sender->len)
                                    == 0))
                continue;
...
int publ_notify(presentity_t *p, str pres_uri, str *body, str *offline_etag, str *rules_doc, str *dialog_body,
                int from_publish)
{
    str *notify_body = NULL;
...
/*	if (p->event->agg_nbody) {
    ...
    while (s) {
        ...
        LM_INFO("notify\n");
        if (notify(s, NULL, notify_body ? notify_body : body, 0, p->extra_hdrs ? p->extra_hdrs : &notify_extra_hdrs,
                   from_publish) < 0) {
...
int query_db_notify(str *pres_uri, pres_ev_t *event, subs_t *watcher_subs)
{
    ...
    if (event->type & PUBL_TYPE) {
        notify_body = get_p_notify_body(*pres_uri, event, 0, 0, 0, 0, &notify_extra_hdrs, &free_fct, 0);
    }
...
int send_notify_request(subs_t *subs, subs_t *watcher_subs, str *n_body, int force_null_body, str *extra_hdrs,
                        int from_publish)
{
    dlg_t *td = NULL;
    str met = {"NOTIFY", 6};
    ...
    */
    ...
    if (force_null_body || avail_pub == 0) {
        LM_DBG("----------------------------------------------- NO PUBLISH-----------------------------\n");
        goto jump_over_body;
    }

    if (n_body != 0 && n_body->s != 0 && subs->status == ACTIVE_STATUS) {
        //LM_DBG("In active_status\n");
            ...
            if (subs->event->type & WINFO_TYPE) {
                ...
                if (from_publish && n_body != 0 && n_body->s != 0)
                    notify_body = n_body;
                        ...
                        notify_body = get_p_notify_body(subs->pres_uri, subs->event, 0, 0,
                                                        (subs->contact.s) ? &subs->contact : NULL, NULL,
                                                        extra_hdrs ? extra_hdrs : &notify_extra_hdrs, &free_fct,
                                                        from_publish);
                }
    ...
    result = tmb.t_request_within(&met, /* method*/
    ...
    /*LM_DBG(
            "NOTIFY %.*s via %.*s on behalf of %.*s for event %.*s, to_tag=%.*s, cseq=%d\n",
            td->rem_uri.len, td->rem_uri.s, td->hooks.next_hop->len,
            td->hooks.next_hop->s, td->loc_uri.len, td->loc_uri.s,
            subs->event->name.len, subs->event->name.s, td->id.loc_tag.len,
            td->id.loc_tag.s, td->loc_seq.value);*/
    ...
    if ((int) (long) n_body != (int) (long) notify_body) {
        if (notify_body != NULL) {
            if (notify_body->s != NULL) {
                if (subs->event->type & WINFO_TYPE)
                    xmlFree(notify_body->s);
                else if (subs->event->apply_auth_nbody == NULL && subs->event->agg_nbody == NULL)
                    pkg_free(notify_body->s);
                else
                    subs->event->free_body(notify_body->s);
            }
...
int notify(subs_t *subs, subs_t *watcher_subs, str *n_body, int force_null_body, str *extra_hdrs, int from_publish)
{
    ...
    if (subs->expires != 0 && subs->status != TERMINATED_STATUS) {
        ...
        hash_code = core_hash(&subs->pres_uri, &subs->event->name,
                shtable_size);*/
        //  LM_DBG("********************************2\n");
        if (updateSubscriptionInCache(subs, LOCAL_TYPE) < 0) {
...
void p_tm_callback(struct cell *t, int type, struct tmcb_params *ps)
{
    ...
    if (ps->param == NULL || *ps->param == NULL || ((c_back_param *) (*ps->param))->pres_uri.s == NULL ||
        ((c_back_param *) (*ps->param))->ev_name.s == NULL || ((c_back_param *) (*ps->param))->to_tag.s == NULL) {
    ...
    if (ps->code == 200) {
        /*LM_DBG("completed with status [%d] and to_tag [%.*s]\n", ps->code,
                cb ? cb->to_tag.len : 0, cb ? cb->to_tag.s : "");*/
    }
    else {
        LM_WARN("completed with status [%d] and to_tag [%.*s], cseq [%.*s]\n", ps->code, cb ? cb->to_tag.len : 0,
                cb ? cb->to_tag.s : "", t->cseq_n.len, t->cseq_n.s);
    }
...
c_back_param *shm_dup_cbparam(subs_t *subs)
{
    ...
    size = sizeof(c_back_param) + subs->pres_uri.len + subs->event->name.len + subs->to_tag.len;
...
str *create_winfo_xml(watcher_t *watchers, char *version, str resource, str event, int STATE_FLAG)
{
    ...
    if (STATE_FLAG & FULL_STATE_FLAG) {
        if (xmlNewProp(root_node, BAD_CAST "state", BAD_CAST
        "full") == NULL) {
    ...
    w_list_node = xmlNewChild(root_node, NULL, BAD_CAST
    "watcher-list", NULL);
    if (w_list_node == NULL) {
    ...
    xmlNewProp(w_list_node, BAD_CAST
    "resource", BAD_CAST
    buffer);
    pkg_free(buffer);
    ...
    while (w) {
        ...
        content[w->uri.len] = '\0';
        node = xmlNewChild(w_list_node, NULL, BAD_CAST
        "watcher", BAD_CAST
        content);
        if (node == NULL) {
            LM_ERR("while adding child\n");
        ...
        if (xmlNewProp(node, BAD_CAST "id", BAD_CAST
        w->id.s)== NULL) {
            LM_ERR("while adding new attribute\n");
            goto error;
        }

        if (xmlNewProp(node, BAD_CAST "event", BAD_CAST
        "subscribe")== NULL) {
            LM_ERR("while adding new attribute\n");
            goto error;
        }

        if (xmlNewProp(node, BAD_CAST
            "status", BAD_CAST
        get_status_str(w->status))== NULL) {
            LM_ERR("while adding new attribute\n");
            goto error;
        }
        w = w->next;
